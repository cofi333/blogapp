...
export const checkReactionStatus = async (
  userId: string,
  commentId: string,
) => {
  try {
...
export const insertReaction = async (
  ...
  try {
    ...
    if (!isAlreadyReacted.data)
      await db
        .insert(reactions)
        .values({ userId: userId, commentId: commentId, reaction: reaction });
    else if (isAlreadyReacted.data.reaction === reaction)
      await updateReactionStatus(userId, commentId, null);
    else await updateReactionStatus(userId, commentId, reaction);
...
export const getReactions = async (commentId: string, userId: string) => {
  try {
    const queryLikes = await db
      .select({
        likes: count(),
      })
      .from(reactions)
      .where(
        and(eq(reactions.reaction, 1), eq(reactions.commentId, commentId)),
      );

    const queryDislikes = await db
      .select({
        dislikes: count(),
      })
      .from(reactions)
      .where(
        and(eq(reactions.commentId, commentId), eq(reactions.reaction, 0)),
      );

    const queryStatus = await db
      .select()
      .from(reactions)
      .where(
        and(eq(reactions.commentId, commentId), eq(reactions.userId, userId)),
      );
